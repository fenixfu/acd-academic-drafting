...
3. **启动 opencode 并加载 skill**
   ...
   opencode
   ```
   启动提示词：请仔细阅读`references/plan_example.md`，将项目流程写入todos，然后开始执行

   `plan_example.md`是“已附魔的执行计划”，制作方法：首先要求agent深入理解项目逻辑和流程、关键步骤和执行红线，在此基础上输出比大的步骤详细一个层级的执行计划。然后再要求agent将AGENTS.md中的“执行红线”补充标注到执行计划的所有相应步骤之后。此过程在正式启动之前完成，不占用主会话的上下文。
