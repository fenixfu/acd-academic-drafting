...
将原始讨论内容转化为可直接用于写作的**结构化素材库**，每条素材都要标注「原始表述→学术表述」的转化过程。此任务**必须**由主agent完成，**严禁**委托subagent完成，但查找相关资料不在此列。
...
#### 1. 拟定备选标题（生成3个，让用户决定）
此任务**必须委托subagent完成**。执行方法：向subagent提供征稿要求和讨论成果，并传递下面两节“标题拟定方法”和“标题质量评估方法”，令其生成3个备选标题，然后主agent转交给用户评估和确认。
...
根据目标字数，设计合适的层级结构。此任务**必须由主agent完成**，**严禁**委托subagent完成（但查找相关资料不在此列）。
